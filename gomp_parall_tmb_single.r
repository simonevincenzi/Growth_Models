...
pacman::p_load("tidyverse", "data.table", "parallel", "MASS", "TMB", "lubridate", "devtools", "Metrics", "rlist","withr")
...
all_pop_df_all = all_pop_df  
long_all_pop_df_all = long_all_pop_df

single_pop_v = c("LIdri_MT","LIdri_RT","UIdri_MT","UVol_BT")

seed_v = c(5,6,7,8,9)

for (sp in 1:length(single_pop_v)) {

all_pop_df = filter(all_pop_df_all, Pop == single_pop_v[sp])
long_all_pop_df = filter(long_all_pop_df_all, Pop == single_pop_v[sp])
...
for (ss in 1:length(seed_v)) {
  ...
  mod_comb_df= expand.grid(mod_comb_df) 
...
#data_region_list_full = data_region_list_full[c(1,50,120)]

max <- 20
x <- 1:length(data_region_list_full)
d1 <- split(1:length(data_region_list_full), ceiling(x/max))

single_name = paste("ll_list_temp_gomp_single_",single_pop_v[sp],".RDS",sep="")
ll_list_temp_single = list()
for (i in 1:length(d1)) {
  ...
  ll_list_temp_single = c(ll_list_temp_single,ll_list_temp_chunk)
  rm(ll_list_temp_chunk)
  saveRDS(ll_list_temp_single,single_name)
  cat(cat(as.character(Sys.time()), file=logFile, append=TRUE, sep = "\n"), file=logFile, append=TRUE, sep = "\n")
}

single_name = paste("ll_list_temp_vb_single_",single_pop_v[sp],".RDS",sep="")
ll_list_temp_single = list() 
for (i in 1:length(d1)) {
