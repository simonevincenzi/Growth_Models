vB_TMB_parall_validation_choice_rand_choice_cov.f = function(data_region_df = data_region_df, 
 ...
 w = NA, linf = NA,
  ...
  if (file.exists("scripts/m_grow3_TMB_daily.o") == F){
    compile("scripts/m_grow3_TMB_daily.cpp")
  }
  dyn.load(dynlib("scripts/m_grow3_TMB_daily"))
  ...
  if (rand_eff_n == 3) {
    ...
    # Build the model
    obj <- MakeADFun(data=dat,parameters = parameters, random=c("u","v","w"),DLL="m_grow3_TMB_daily")
    opt = NULL
    ...
    # Build the model
    obj <- MakeADFun(data=dat,parameters = parameters, random=c("u","v"),DLL="m_grow3_TMB_daily", map = map)
    opt = NULL
